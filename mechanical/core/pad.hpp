...
/// @since 20130925
/// @date 20250401
class Pad : public ServiceBase<IPad> {
  private:
    ...
    void added(void) {
        ...
        if (joysticks) {
            for (i = 0; i < num_joysticks; ++i) {
                ...
                SDL_Log("Gamepad %" SDL_PRIu32 ": %s%s%s VID 0x%.4x, PID 0x%.4x", instance_id, name ? name : "Unknown",
                        path ? ", " : "", path ? path : "", SDL_GetGamepadVendorForID(instance_id),
