...
struct Uniform {
    ...
    Uniform(const int& val) : type(UniformType::INT), ptr(std::static_pointer_cast<void>(std::make_shared<int>(val))) {}
    ...
    Uniform(const std::vector<float>& val)
        : type(UniformType::FLOAT_VEC), ptr(std::static_pointer_cast<void>(std::make_shared<std::vector<float>>(val))) {
    }
