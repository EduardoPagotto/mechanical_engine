...
namespace me {
    ...
    Texture::Texture(const uint32_t& width, const uint32_t& height, const TexParam& tp)
        ...
        this->init();
        glTexImage2D(GL_TEXTURE_2D, 0, (GLuint)textureParameters.internalFormat, width, height, 0,
                     (GLuint)textureParameters.format, (GLuint)textureParameters.type, nullptr);
    }
    ...
    Texture::Texture(SDL_Surface* surface, const TexParam& tp)
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, (GLuint)textureParameters.format, surface->w, surface->h, 0,
    ...
    void Texture::invert_image_texture(int pitch, int height, void* image_pixels) {
        ...
        for (index = 0; index < height_div_2; index++) {
            ...
            memcpy((Uint8*)(image_pixels) + pitch * index, (Uint8*)(image_pixels) + pitch * (height - index - 1),
                   pitch);
